...
public class StateController4_4 : MonoBehaviour {
...
private const string RIGHT_HAND_NAME = "Human_RightHand"; 
    ...
    IEnumerator waiter()
    {
        ...
        if(!activeTimer) {
            ...
            activeTimer = true; // evitar la concurrencia entre threads. 
            state = 1; 
            fuenteAudio.Play(); 
        ...
        if(checkIfColliderAvailable(this.name) == true) { 
            ...
            switch(this.name) {
                ...
                case "aux1": 
                    ...
                    break; 
                case "state2": 
                    handlerCommonBoxes(2, 2); // estado 2
                    break; 
                case "aux2": 
                    handlerCommonBoxes(2, 3); // estado 2
                    break; 
                case "aux3": 
                    handlerMiddleBox(); // estado 2
                    break; 
                case "state3": 
                    handlerCommonBoxes(3, 5); // estado 3
                    break;  
                case "aux4": 
                    handlerCommonBoxes(3, 6); // estado 3
                    break; 
                case "state4": 
                    handlerCommonBoxes(4, 7); // estado 4
                    break; 
                case "aux5": 
                    handlerFinalBox();  // estado 4
                    break; 
    ...
    private void handlerState1() {
        if(state == 1) {
            if(afterMove == false) {
                ...
                if(okStates.Count == 9) { // si todos los movimientos han sido correctos
                    feedbackGoodGesture(); 
    ...
    private void handlerCommonBoxes(int s, int prevBoxes) {
        if(prevBoxes > 4) { // ya has pasado middleBox.
